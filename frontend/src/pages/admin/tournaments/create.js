...
import { createTournament, getLeagues, getTeams, getPlayers, getPlayerLeagueStatuses } from '../../../utils/api';
...
export default function CreateTournamentPage() {
  ...
  const [teamPlayers, setTeamPlayers] = useState({}); // { teamId: { [playerId]: { checked, number, is_foreign, is_self, registered, unregistered } } }
  ...
  const handleTourDateChange = (tourNumber, date) => {
    setTourDates(prev => ({
      ...prev,
      [tourNumber]: date
    }));
  ...
  function handlePlayerCheckbox(teamId, player) {
    setTeamPlayers(tp => {
      ...
      if (current[player.id]?.checked) {
        ...
        // добавляем игрока с дефолтными значениями
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-4">
        <h1 className="text-2xl font-bold">Добавить турнир</h1>
        <button 
